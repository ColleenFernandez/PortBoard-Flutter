...
class SubmitIFTAStickerPage extends StatefulWidget {
  @override
  State<SubmitIFTAStickerPage> createState() => _SubmitIFTAStickerPageState();
}

class _SubmitIFTAStickerPageState extends State<SubmitIFTAStickerPage> {

  TextEditingController edtIFTASticker = new TextEditingController();
  ...
  void loadData(){
    if (Common.userModel.iftaStickerModel.status == Constants.PENDING || Common.userModel.iftaStickerModel.status == Constants.ACCEPT) {
      isEditable = false;
    }else {
      isEditable = true;
    }

    edtIFTASticker.text = Common.userModel.iftaStickerModel.iftaStickerNumber;
    state = Common.userModel.iftaStickerModel.state;
    edtExpiryDate.text = Utils.getDate(Common.userModel.iftaStickerModel.expirationDate);
    frontPic = Common.userModel.iftaStickerModel.frontPic;
  }

  void submitIFTASticker() async{

    final frontPicFile = frontPic as File;
    final String frontPicPath = await FlutterAbsolutePath.getAbsolutePath(frontPicFile.path);

    showProgress();
    Common.api.submitIFTASticker(
        Common.userModel.id,
        edtIFTASticker.text, state, expiryDate.toString(), frontPicPath).then((value){
          closeProgress();
          if (value == APIConst.SUCCESS) {
            showSingleButtonDialog(
                context,
                'IFTA Stickers Detail Submitted!',
                'Your IFTA Stickers detail submitted successfully!\nAdministrator will check it and reply you as soon as possible.',
                Constants.Okay, () {
  ...
  bool isValid(){
    ...
    if (edtIFTASticker.text.isEmpty){
      showToast('Please input IFTA Sticker number');
      return false;
    }
    ...
    return Scaffold(
          ...
          children: [
            ...
            Container(
                ...
                decoration: BoxDecoration(
                    ...
                    color: Colors.white
                ), child: Text(Utils.getStatus(Common.userModel.iftaStickerModel.status), style: TextStyle(color: AppColors.darkBlue),))
          ],
        ),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Visibility(
              visible: Common.userModel.iftaStickerModel.status == Constants.REJECT,
              child: Container(
                  ...
                  child: Text(Common.userModel.iftaStickerModel.reason, style: TextStyle(color: Colors.red),)),
            ),
            Text('IFTA Sticker Number'),
            TextField(controller: edtIFTASticker,),
            SizedBox(height: 30,),
            Row(
            ...
            Visibility(
              visible: Common.userModel.iftaStickerModel.status != Constants.ACCEPT,
              child: Container(
